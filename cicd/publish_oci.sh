...
# Check if a properties.json file that was generated by the build
ZOPEN_IMAGE_CONTAINERFILE_NAME="Containerfile"
ZOPEN_PODMAN_BINARY="podman"

# Parse the properties.json file and extract the necessary information
PRODUCT_NAME=$(jq -r '.product.name' properties.json)
PRODUCT_PAX=$(jq -r '.product.pax' properties.json)
PRODUCT_VERSION=$(jq -r '.product.version' properties.json)
PRODUCT_RELEASE=$(jq -r '.product.release' properties.json)
